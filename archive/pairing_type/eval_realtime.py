"""Forced-choice eval of gpt-realtime on sewn pairing_type clips.

Each MCQ clip is sent as audio. The model must answer A/B/C/D for
Speaker B's current status. Options use display labels only (no laugh /
groan / contrast names).

Usage:
    python pairing_type/eval_realtime.py
    python pairing_type/eval_realtime.py --limit 1
    python pairing_type/eval_realtime.py --seed 0 --resume
"""
...
SESSION_INSTRUCTIONS = (
    "You are taking a listening test. You will hear a short two-speaker "
    "conversation. Speaker A speaks first. Speaker B speaks second. Decide "
    "Speaker B's current status from the four choices in the user message. "
...
def option_block(question: dict) -> str:
    ...
    lines.extend(["", "Reply with only one letter: A, B, C, or D."])
    return "\n".join(lines)
...
def parse_letter(text: str) -> str | None:
    ...
    if text.upper() in {"A", "B", "C", "D"}:
        return text.upper()
    m = re.match(r"^\s*([ABCD])(?:\s*[.):,\-]|\s*$)", text, re.I)
    if m:
...
def summarize(rows: list[dict]) -> dict:
    return {
        ...
        "by_contrast": accuracy_block(rows, "comparison_id"),
        "by_version": accuracy_block(rows, "version"),
        "by_domain": accuracy_block(rows, "domain"),
        "by_correct_label": accuracy_block(rows, "correct_label"),
    }
...
def print_summary(summary: dict) -> None:
    print("\n=== gpt-realtime accuracy ===")
    print(f"overall          {fmt_acc(summary['overall'])}")
    print("by contrast")
    for name, block in sorted(summary["by_contrast"].items()):
        print(f"  {name:<16} {fmt_acc(block)}")
    print("by version")
    for name, block in sorted(summary["by_version"].items()):
        print(f"  {name:<16} {fmt_acc(block)}")
    print("by domain")
    for name, block in sorted(summary["by_domain"].items()):
        print(f"  {name:<16} {fmt_acc(block)}")
    print("by gold label")
    for name, block in sorted(summary["by_correct_label"].items()):
        print(f"  {name:<24} {fmt_acc(block)}")
...
def main() -> None:
    ...
    mcq = json.loads(args.mcq.read_text())
    questions = list(mcq["results"])
    ...
    for i, question in enumerate(questions, start=1):
        ...
        label = next(
            (opt["label"] for opt in question["options"] if opt["key"] == gold),
            None,
        )
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            try:
                ...
                row = {
                    ...
                    "comparison_id": question["comparison_id"],
