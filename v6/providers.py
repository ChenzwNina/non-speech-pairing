...
# 2.0 tests every model at its own default effort and thinking level, per the spec. 1.0 raised
# the tested models to high; that is deliberately dropped here, so nothing in this file sets
# reasoning effort or a thinking level and each provider gets whatever it does unprompted.
MODELS = {
...
# Measured 2026-08-26; see 1.0/PIPELINE.md, Known problems.
GROK_MANUAL_TURN = True

# One deterministic failure survives: the audio commits, then response.create is dropped in
# silence -- no response.created, no error, no response.done. Nothing in the protocol says it
# happened, so the only remedy is a bounded wait and a fresh session.
GROK_FIRST_BYTE_GRACE = 10.0
GROK_ATTEMPTS = 3


# Every call appends {provider, input_tokens, output_tokens}. Grok reported nothing at all in
# 1.0 — input_tokens, output_tokens and total_tokens all null — so an empty row for grok is
# the API's silence, not a bug here.
USAGE: list[dict] = []
...
def ask_openai_many(audios: list[Path], task: str, question: str, model: str) -> str:
    """Several recordings in one session, as separate content parts of one message."""
    import sys
    sys.path.insert(0, str(REPO / "archive" / "logo_sketch"))
    import eval_realtime as ev
    import time
    ...
    try:
        ...
        ev.wait_for(conn, deadline, "session.updated")
        content = [{"type": "input_audio",
                    "audio": base64.b64encode(pcm(path, RATE_24K)).decode("ascii")}
                   for path in audios]
        if question:
            content.append({"type": "input_text", "text": question})
        conn.conversation.item.create(item={"type": "message", "role": "user",
...
async def _ask_gemini_many(audios: list[Path], task: str, question: str, model: str) -> str:
    """Several recordings in one session, each as its own activity block."""
    ...
    async with client.aio.live.connect(model=model, config=config) as session:
        for path in audios:
            data = pcm(path, RATE_16K)
            await session.send_realtime_input(activity_start=types.ActivityStart())
            for i in range(0, len(data), 3200):
                await session.send_realtime_input(
                    audio=types.Blob(data=data[i:i + 3200],
...
def ask_gemini_many(audios: list[Path], task: str, question: str, model: str) -> str:
    return asyncio.run(_bounded(_ask_gemini_many(audios, task, question, model),
                                GEMINI_TIMEOUT))


ASK_MANY = {"openai": ask_openai_many, "gemini": ask_gemini_many}


def ask_many(provider: str, audios: list[Path], task: str, question: str,
             model: str | None = None) -> str:
    """Play several recordings to one model in one session and return what it says back.

    Only the two providers that judge tone implement this. Joining the recordings into one
    file would be simpler, but a judge then has to find the boundary itself — and the last
    turn before it is the one carrying the vocalization, so a judge that drifts rates the
    stimulus instead of the reply, in the direction of the stimulus being the more marked of
    the two.
    """
    if provider not in ASK_MANY:
        raise NotImplementedError(
            f"{provider} takes one recording per session; only {sorted(ASK_MANY)} accept "
            f"several")
    return ASK_MANY[provider](audios, task, question, model or MODELS[provider])
...
def _ask_grok_once(audio: Path, task: str, question: str, model: str) -> str:
    import sys, time
    ...
    with client.realtime.connect(model=model) as conn:
        conn.session.update(session={
            # Manual turn control. Under server VAD grok discarded whole clips and answered
            # "no audio clip is provided" — which reads exactly like a confident NO unless you
            # ask it for a reason, which is how 1.0's 190 verdicts came to be void. With
            # turn_detection off and an explicit commit, 11 of 12 clips answered correctly.
            "voice": "eve", "instructions": task,
            "turn_detection": None if GROK_MANUAL_TURN else
        ...
        conn.response.create(response={"output_modalities": ["text"]})
        created = gv.wait_until(conn, {"response.created"},
                                min(deadline, time.time() + GROK_FIRST_BYTE_GRACE), seen)
...
def _grok_retry(call, *args):
    """Grok drops response.create in silence on some inputs; a fresh session usually takes.

    The failure is invisible in the protocol — no error, no response.done — so there is
    nothing to catch but the absence of a reply, which is why the wait is bounded rather
    than left to the overall timeout.
    """
    import time
    last = None
    for attempt in range(1, GROK_ATTEMPTS + 1):
        try:
            return call(*args)
        except (TimeoutError, RuntimeError) as exc:
            last = exc
            if attempt < GROK_ATTEMPTS:
                time.sleep(1.5 * attempt)
    raise last


def ask_grok(audio: Path, task: str, question: str, model: str) -> str:
    return _grok_retry(_ask_grok_once, audio, task, question, model)


def converse_grok(audio: Path, task: str, followups: list[str], model: str) -> dict:
    return _grok_retry(_converse_grok_once, audio, task, followups, model)
...
def _converse_grok_once(audio: Path, task: str, followups: list[str],
                        model: str) -> dict:
    import sys, time
    sys.path.insert(0, str(REPO / "archive" / "printer_jam"))
    import eval_grok_voice as gv
    ...
    with client.realtime.connect(model=model) as conn:
        ...
        if GROK_MANUAL_TURN:
            # Close the turn ourselves. No silence tail: VAD is off, so nothing is listening
            # for one, and the tail was only ever there to make VAD let go.
            conn.input_audio_buffer.commit()
        else:
            gv.append_pcm(conn, b"\x00\x00" * int(RATE_24K * 1.0), seen, asr, realtime=False)
...
def _grok_turn(conn, gv, deadline: float, seen: list, modalities: list) -> tuple[str, bytes]:
    ...
    import time as _t
    created = gv.wait_until(conn, {"response.created"},
                            min(deadline, _t.time() + GROK_FIRST_BYTE_GRACE), seen)
